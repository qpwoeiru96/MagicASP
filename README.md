...
注意1：回档功能以及在线调试需保证 主入口配置 debug 始终为 false

以及runtime下生成的全部文件完好无缺,注意是全部文件,旧的代码全部在里面，并且保证所有module跟action的文件均生产。
...
通过分析QueryString取得m跟a的值。

比如链接：http://www.example.com/index.asp?_=user/register/&name=demo 那么m=user a=register
根据需要进行日志记录。
